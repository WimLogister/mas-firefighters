...
public class CheckWeatherExtinguishFirePlan extends Plan{
	...
	public void executeNextStep(Agent agent) {
		//System.out.println("Execute weather plan");
		if(getSteps().get(0) instanceof CheckWeather){
			...
			this.predictedFireLocation = calculateNewFirePosition(weather);
			System.out.println(agent.getGrid().getLocation(agent).getX() + " ~~::~~ " + agent.getGrid().getLocation(agent).getY());
			System.out.println(predictedFireLocation.getX() + " --::-- " + predictedFireLocation.getY());
			//System.out.println("New fire location " + predictedFireLocation.getX() + " :: " + predictedFireLocation.getY());
			...
			System.out.println("size " + pathToNewLoc.getRoute().size());
			...
			//System.out.println("not checking the weather");
			getSteps().remove(0).execute(agent);	  
		}
	}
	...
	public GridPoint calculateNewFirePosition(WeatherInformation weather){
		...
		int noOfSteps = pathToFire.getRoute().size()/2;	     
		...
		else return actualFireLocation;
	}
}
